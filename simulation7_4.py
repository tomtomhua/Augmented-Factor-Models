...
for oi in omega_list:
    ...
    for i in range(K):
        for j in range(K):
            PI1[i,j] = 0.5 if i == j else 0.4**np.abs(i-j)
    PI2 = np.zeros((N,N))
    for i in range(N):
        for j in range(N):
            PI2[i,j] = 0.5 if i == j else 0.4**np.abs(i-j)
    
    def generate_dependent(x0,PI,T):
        x_out = [x0]
        for _ in range(T-1):
            x1 = np.matmul(x0,PI) + np.random.normal(0,1,len(x0))
            x_out.append(x1)
            x0 = x1
        return(np.array(x_out).transpose())
    
    u_t_normal = generate_dependent(np.random.normal(0,1,N),PI2,T)
    u_t_normal_log = generate_dependent(np.random.lognormal(0,1,N),PI2,T)
    x_t = generate_dependent(np.random.normal(0,1,K),PI1,T)
...
df1.to_csv('result/single_replication_table_7_4.csv')
